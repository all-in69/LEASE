...
// Nowe importy z biblioteki recharts
import { PieChart, Pie, Cell, Tooltip, Legend, ResponsiveContainer } from 'recharts';

const VAT_RATE = 1.23;
const CHART_COLORS = ['#0088FE', '#00C49F', '#FFBB28']; // Kolory dla wykresu

const Calculator = () => {
    // Stany (bez większych zmian)
    ...
    // --- NOWY STAN DLA TARCZY PODATKOWEJ ---
    const [taxRate, setTaxRate] = useState(19);

    const handleModeChange = (newMode) => setMode(newMode);

    // Zaktualizowana funkcja obliczeniowa
    const runCalculation = useCallback(async () => {
        ...
        // --- POPRAWKA: Dodatkowa walidacja po stronie klienta ---
        if (mode === 'rate') {
            if (carValueNum <= 0 || leaseTermNum <= 0) {
                setResult(null); // Czyść wynik, jeśli dane są nieprawidłowe
                return;
            }
        ...
        try {
            setError(''); // Wyczyść poprzedni błąd przed nowym zapytaniem
            ...
            // --- POPRAWKA: Wyświetlanie bardziej szczegółowego błędu ---
        ...
        <div className="calculator">
            ...
            <div className="form-grid">
                <div className="input-column">
                    ...
                    <div className="form-group"><label>Marża (%)</label><input type="number" step="0.1" value={margin} onChange={(e) => setMargin(e.target.value)} /></div>
                    <div className="form-group"><label>WIBOR (%)</label><input type="number" step="0.1" value={wibor} onChange={(e) => setWibor(e.target.value)} /></div>
